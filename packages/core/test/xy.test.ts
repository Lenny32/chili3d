...
describe("XY class", () => {
    describe("static properties", () => {
        test("should have correct zero", () => {
            expect(XY.zero).toEqual(new XY({ x: 0, y: 0 }));
        });

        test("should have correct unitX", () => {
            expect(XY.unitX).toEqual(new XY({ x: 1, y: 0 }));
        });

        test("should have correct unitY", () => {
            expect(XY.unitY).toEqual(new XY({ x: 0, y: 1 }));
        });
    });

    describe("constructor", () => {
        test("should create XY with given values", () => {
            const xy = new XY({ x: 3, y: 4 });
            expect(xy.x).toBe(3);
            expect(xy.y).toBe(4);
        });

        test("should create XY with negative values", () => {
            const xy = new XY({ x: -1, y: -2 });
            expect(xy.x).toBe(-1);
            expect(xy.y).toBe(-2);
        });
    ...
    describe("cross", () => {
        test("should calculate cross product correctly", () => {
            const a = new XY({ x: 1, y: 0 });
            const b = new XY({ x: 0, y: 1 });
        ...
        test("should return 0 for parallel vectors", () => {
            const a = new XY({ x: 2, y: 0 });
            const b = new XY({ x: 3, y: 0 });
    ...
    describe("dot", () => {
        test("should calculate dot product correctly", () => {
            const a = new XY({ x: 1, y: 2 });
            const b = new XY({ x: 3, y: 4 });
    ...
    describe("divided", () => {
        test("should divide by scalar", () => {
            const xy = new XY({ x: 4, y: 6 });
            const result = xy.divided(2);
            expect(result).toEqual(new XY({ x: 2, y: 3 }));
        });
    ...
    describe("reverse", () => {
        test("should reverse vector", () => {
            const xy = new XY({ x: 3, y: 4 });
            expect(xy.reverse()).toEqual(new XY({ x: -3, y: -4 }));
        });
    ...
    describe("multiply", () => {
        test("should multiply by scalar", () => {
            const xy = new XY({ x: 2, y: 3 });
            expect(xy.multiply(3)).toEqual(new XY({ x: 6, y: 9 }));
        });

        test("should handle negative scalar", () => {
            const xy = new XY({ x: 2, y: 3 });
            expect(xy.multiply(-1)).toEqual(new XY({ x: -2, y: -3 }));
        });
    });

    describe("sub", () => {
        test("should subtract vectors", () => {
            const a = new XY({ x: 5, y: 7 });
            const b = new XY({ x: 2, y: 3 });
            expect(a.sub(b)).toEqual(new XY({ x: 3, y: 4 }));
        });
    });
    ...
    describe("add", () => {
        test("should add vectors", () => {
            const a = new XY({ x: 1, y: 2 });
            const b = new XY({ x: 3, y: 4 });
            expect(a.add(b)).toEqual(new XY({ x: 4, y: 6 }));
        });
    });
    ...
    describe("normalize", () => {
        ...
        test("should return undefined for very small vector", () => {
            const xy = new XY({ x: 1e-10, y: 1e-10 });
    ...
    describe("center", () => {
        test("should calculate center correctly", () => {
            const a = new XY({ x: 0, y: 0 });
            const b = new XY({ x: 4, y: 6 });
            expect(XY.center(a, b)).toEqual(new XY({ x: 2, y: 3 }));
        });
    });
    ...
    describe("angleTo", () => {
        ...
        test("should return 0 for same direction", () => {
            const a = new XY({ x: 1, y: 0 });
            const b = new XY({ x: 2, y: 0 });
            expect(a.angleTo(b)).toBeCloseTo(0);
        });

        test("should return PI for opposite direction", () => {
            const a = new XY({ x: 1, y: 0 });
            const b = new XY({ x: -1, y: 0 });
            expect(a.angleTo(b)).toBeCloseTo(Math.PI);
        });

        test("should return undefined for zero vector", () => {
            expect(XY.zero.angleTo(new XY({ x: 1, y: 0 }))).toBeUndefined();
            expect(new XY({ x: 1, y: 0 }).angleTo(XY.zero)).toBeUndefined();
        });
    });
    ...
    describe("isParallelTo", () => {
        ...
        test("should return false for non-parallel vectors", () => {
            const a = new XY({ x: 1, y: 0 });
            const b = new XY({ x: 1, y: 1 });
            expect(a.isParallelTo(b)).toBe(false);
        });

        test("should return undefined for zero vector", () => {
            expect(XY.zero.isParallelTo(new XY({ x: 1, y: 0 }))).toBeUndefined();
        });
    });
    ...
    describe("isOppositeTo", () => {
        ...
        test("should return undefined for zero vector", () => {
            expect(XY.zero.isOppositeTo(new XY({ x: 1, y: 0 }))).toBeUndefined();
        });
    });
});
