// Part of the Chili3d Project, under the AGPL-3.0 License.
// See LICENSE file in the project root for full license information.

import type { Locale } from "@chili3d/core";

export default {
    display: "简体中文",
    language: "zh-CN",
    translation: {
        "ai.apiKey": "API Key",
        "ai.apiKeyHint": "密钥不会写入浏览器存储。可在密码管理器提示时保存，否则每次会话需输入一次。",
        "ai.ask.notAnswered": "（未作答）",
        "ai.ask.placeholder": "输入回答…",
        "ai.baseURL": "Base URL",
        "ai.cancel": "取消",
        "ai.copy": "复制",
        "ai.emptyAsk": "也可以直接问 Chili3D 怎么用，例如：「怎么给边倒斜角？」",
        "ai.emptyExample": "例如：创建一个 10×20×5 的盒子，再倒 2mm 圆角",
        "ai.emptyHint": "用自然语言创建和编辑 3D 模型",
        "ai.emptyTitle": "AI 建模助手",
        "ai.error.missingParam": '缺少必填参数 "{0}"',
        "ai.error.noDocument": "没有打开的文档 —— 请先新建或打开一个文档",
        "ai.error.noRefs": "（尚未注册任何引用）",
        "ai.error.nullRef": '引用 "{0}" 已定义，但其查询结果为 null，不能用作 target 或参数',
        "ai.error.prefix": "错误：{0}",
        "ai.error.unknownRef":
            '未知的引用 "{0}"。当前可用引用：{1} —— 引用只在定义它的 op 执行成功后存在，请检查 op 顺序，不要凭编号规律臆测 id。',
        "ai.history": "历史对话",
        "ai.historyDelete": "删除「{0}」？",
        "ai.historyEmpty": "暂无历史对话",
        "ai.inputPlaceholder": "输入建模需求，例如：创建一个 10×20×5 的盒子",
        "ai.model": "模型",
        "ai.newChat": "新建对话",
        "ai.noReply": "（无文本回复）",
        "ai.provider": "服务商",
        "ai.save": "保存",
        "ai.send": "发送",
        "ai.sendHint": "发送（Ctrl+Enter）",
        "ai.settings": "设置",
        "ai.settingsTitle": "AI 助手设置",
        "ai.stepLimit": "（本轮步数已达上限——回复「继续」我就接着做下去。）",
        "ai.stop": "停止",
        "ai.title": "AI 助手",
        "ai.tool.created": "已创建 {0}",
        "ai.tool.removed": "已删除 {0}",
        "ai.untitled": "新对话",
        "ai.workedFor": "已处理 {0}",
        "ai.working": "处理中…",
        "arc.angle": "角度",
        "arc.start": "起点",
        "axis.x": "X 轴",
        "axis.y": "Y 轴",
        "axis.z": "Z 轴",
        "body.arc": "圆弧",
        "body.bolean": "布尔",
        "body.box": "立方体",
        "body.circle": "圆形",
        "body.cone": "圆锥",
        "body.cylinder": "圆柱",
        "body.editableShape": "可编辑的形状",
        "body.ellipse": "椭圆",
        "body.face": "面",
        "body.helix": "螺旋线",
        "body.fuse": "合并",
        "body.group": "组",
        "body.imported": "导入的模型",
        "body.line": "直线",
        "body.meshNode": "网格节点",
        "body.multiShape": "多形状",
        "body.parametricBody": "参数化实体",
        "body.point": "点",
        "body.polygon": "多段线",
        "body.regularPolygon": "正多边形",
        "body.pipe": "管状体",
        "body.extrude": "挤出",
        "body.pyramid": "棱锥",
        "body.rect": "矩形",
        "body.revol": "旋转",
        "body.sketch": "草图",
        "body.sphere": "球体",
        "body.sweep": "扫略",
        "body.wire": "线框",
        "box.dx": "长",
        "box.dy": "宽",
        "box.dz": "高",
        "circle.center": "圆心",
        "circle.radius": "半径",
        "command.act.alignCamera": "从相机创建",
        "command.ai.toggleChat": "MCP",
        "command.boolean.common": "交集",
        "command.boolean.cut": "减去",
        "command.boolean.join": "合并",
        "command.convert.curveProjection": "曲线投影",
        "command.convert.fuse": "融合",
        "command.convert.toCompound": "转组合体",
        "command.convert.toFace": "转为面",
        "command.convert.toShell": "转为壳",
        "command.convert.toSolid": "转为体",
        "command.convert.toWire": "转多段线",
        "command.constraint.coincident": "重合",
        "command.constraint.equal": "相等",
        "command.constraint.fix": "固定",
        "command.constraint.horizontal": "水平",
        "command.constraint.horizontalAlign": "水平对齐",
        "command.constraint.midpoint": "中点",
        "command.constraint.parallel": "平行",
        "command.constraint.perpendicular": "垂直",
        "command.constraint.pointOn": "点在线上",
        "command.constraint.symmetric": "对称",
        "command.constraint.tangent": "相切",
        "command.constraint.vertical": "竖直",
        "command.constraint.verticalAlign": "竖直对齐",
        "command.create.arc": "圆弧",
        "command.create.arc2point": "两点圆弧",
        "command.create.arc3point": "三点圆弧",
        "command.create.arcTTR": "相切相切半径圆弧",
        "command.create.bezier": "贝塞尔曲线",
        "command.create.box": "立方体",
        "command.create.circle": "圆形",
        "command.create.cone": "圆锥",
        "command.create.copyShape": "复制形状",
        "command.create.cylinder": "圆柱",
        "command.create.ellipse": "椭圆",
        "command.create.ellipsoid": "椭球体",
        "command.create.extrude": "挤出",
        "command.feature.extrude": "挤出",
        "command.feature.revolve": "旋转",
        "command.feature.fillet": "圆角",
        "command.feature.chamfer": "倒角",
        "command.feature.fuse": "合并",
        "command.feature.cut": "减去",
        "command.feature.common": "交集",
        "command.feature.variable": "变量",
        "command.feature.reselect": "重新选择",
        "command.create.folder": "文件夹",
        "command.create.group": "创建组",
        "command.create.helix": "螺旋线",
        "command.create.line": "直线",
        "command.create.loft": "放样",
        "command.create.offset": "偏移",
        "command.create.pipe": "管状体",
        "command.create.point": "点",
        "command.create.polygon": "多段线",
        "command.create.regularPolygon": "正多边形",
        "command.create.pyramid": "棱锥",
        "command.create.rect": "矩形",
        "command.create.refSegment": "参考线段",
        "command.create.revol": "旋转",
        "command.create.section": "相交曲线",
        "command.create.sphere": "球体",
        "command.create.sweep": "扫略",
        "command.create.thickSolid": "加厚实体",
        "command.dimension.angle": "角度",
        "command.dimension.distance": "距离",
        "command.dimension.horizontalDistance": "水平距离",
        "command.dimension.pointLineDistance": "点线距离",
        "command.dimension.radius": "半径",
        "command.dimension.verticalDistance": "竖直距离",
        "command.doc.new": "新建文档",
        "command.doc.open": "打开文档",
        "command.doc.save": "保存文档",
        "command.doc.saveToFile": "保存到文件",
        "command.edit.redo": "重做",
        "command.edit.undo": "撤销",
        "command.file.export": "导出",
        "command.file.import": "导入",
        "command.measure.angle": "角度",
        "command.measure.length": "长度",
        "command.measure.select": "选择",
        "command.modify.array": "阵列",
        "command.modify.break": "打断",
        "command.modify.brushAdd": "添加画笔",
        "command.modify.brushClear": "清除画笔",
        "command.modify.brushRemove": "删除画笔",
        "command.modify.chamfer": "倒角",
        "command.modify.deleteNode": "删除",
        "command.modify.explode": "分解",
        "command.modify.extend": "延伸",
        "command.modify.fillet": "圆角",
        "command.modify.mirror": "镜像",
        "command.modify.move": "移动",
        "command.modify.paintBucket": "油漆桶",
        "command.modify.removeFeature": "删除特征",
        "command.modify.removeShapes": "删除子元素",
        "command.modify.rotate": "旋转",
        "command.modify.sew": "缝合",
        "command.modify.shell": "抽壳",
        "command.modify.repairShape": "修复形状",
        "command.modify.simplifyShape": "简化",
        "command.modify.checkShape": "检查形状",
        "command.modify.split": "分割",
        "command.modify.trim": "修剪",
        "command.sketch.arc": "圆弧",
        "command.sketch.circle": "圆",
        "command.sketch.polygon": "正多边形",
        "command.sketch.ellipse": "椭圆",
        "command.sketch.point": "点",
        "sketch.polygon.inscribed": "内接（关闭为外切）",
        "sketch.ellipse.foci": "使用两个焦点",
        "prompt.pickFirstFocus": "选择第一个焦点",
        "prompt.pickSecondFocus": "选择第二个焦点",
        "prompt.pickMajorAxis": "选择长轴端点",
        "prompt.pickEllipsePoint": "选择椭圆上的点（短轴端点可输入半径）",
        "command.sketch.create": "新建草图",
        "command.sketch.enter": "编辑草图",
        "command.sketch.exit": "退出草图",
        "command.sketch.line": "直线",
        "command.sketch.projectEdges": "投影边线",
        "command.sketch.rectangle": "矩形",
        "command.sketch.toggleExternal": "切换外部参考",
        "command.special.last": "__上个命令__",
        "command.test.performance": "性能测试",
        "command.workingPlane.alignToPlane": "对齐到平面",
        "command.workingPlane.fromSection": "从截面创建",
        "command.workingPlane.set": "设置工作平面",
        "command.workingPlane.toggleDynamic": "动态工作平面",
        "common.3DNavigation": "3D导航",
        "common.angle": "角度",
        "common.area": "面积",
        "common.back": "返回",
        "common.cancel": "取消",
        "common.clone": "复制对象",
        "common.color": "颜色",
        "common.confirm": "确定",
        "common.count": "数量",
        "common.delete": "删除",
        "common.dir": "方向",
        "common.general": "常规",
        "common.height": "高度",
        "common.isGroup": "组",
        "common.keepTools": "保留操作对象",
        "common.language": "语言",
        "common.length": "长度",
        "common.expression": "表达式",
        "common.location": "位置",
        "common.material": "材质",
        "common.matrix": "矩阵",
        "common.more": "更多",
        "common.name": "名称",
        "common.normal": "法向量",
        "common.numberX": "X 数量",
        "common.numberY": "Y 数量",
        "common.numberZ": "Z 数量",
        "common.on": "开启",
        "common.opacity": "不透明度",
        "common.removeEdges": "移除边",
        "common.removeFaces": "移除面",
        "common.rename": "重命名",
        "common.xDir": "X 方向",
        "common.shapeType": "形状类型",
        "common.theme.dark": "深色",
        "common.theme.light": "浅色",
        "common.theme.system": "系统",
        "common.theme": "主题",
        "common.thickness": "厚度",
        "common.tolerance": "容差",
        "common.type": "类型",
        "common.volume": "体积",
        "common.warning": "警告",
        "common.trust": "信任",
        "common.dontTrust": "不信任",
        "dialog.title.selectWorkingPlane": "选择工作平面",
        "dialog.title.checkShape": "形状检查结果",
        "dialog.title.enterValue": "输入数值",
        "dialog.checkShape.valid": "有效",
        "dialog.checkShape.invalid": "无效",
        "dialog.checkShape.overallValid": "整体形状有效。",
        "dialog.checkShape.overallInvalid": "整体形状无效 — 以下面存在问题。",
        "dialog.checkShape.faceIndex": "面",
        "dialog.checkShape.validity": "有效性",
        "dialog.checkShape.noFaces": "该形状中没有面。",
        "ellipse.majorRadius": "长轴",
        "ellipse.minorRadius": "短轴",
        "ellipsoid.radiusX": "半径X",
        "ellipsoid.radiusY": "半径Y",
        "ellipsoid.radiusZ": "半径Z",
        "entity.editable": "可编辑实体",
        "entity.parameter": "参数化实体",
        "error.default:{0}": "错误: {0}",
        "error.export.noNodeCanBeExported": "没有可导出的节点",
        "error.import.unsupportedFileType:{0}": "不支持的文件类型: {0}",
        "error.input.cannotInputANumber": "与参照点重合，无法输入 1 个数",
        "error.input.invalidNumber": "输入错误，请输入有效的数字，以,分开",
        "error.input.threeNumberCanBeInput": "参照点为空，只能输入 3 个数",
        "error.input.unsupportedInputs": "超过最大输入数",
        "error.select.twoEdges": "请选择两条相邻的边",
        "file.format": "文件格式",
        "helix.pitch": "螺距",
        "home.recent": "最近项目",
        "home.recent.empty": "暂无最近文档，新建或打开一个文档即可开始。",
        "home.videos": "社区视频",
        "home.videos.recent": "更新记录",
        "home.videos.cases": "案例展示",
        "home.videos.more": "更多",
        "home.welcome": "欢迎使用 chili3d",
        "home.welcome.subtitle": "在浏览器中进行 3D 设计与建模。",
        "items.header": "项目",
        "items.tool.delete": "删除",
        "items.tool.expandAll": "展开所有",
        "items.tool.newFolder": "文件夹",
        "items.tool.unexpandAll": "折叠所有",
        "line.end": "终点",
        "line.start": "起点",
        "line.type.line": "直线",
        "line.type.xline": "构造线",
        "material.bumpMap": "凹凸贴图",
        "material.emissive": "自发光",
        "material.emissiveMap": "自发光贴图",
        "material.map": "贴图",
        "material.metalness": "金属度",
        "material.metalnessMap": "金属度贴图",
        "material.normalMap": "法线贴图",
        "material.roughness": "粗糙度",
        "material.roughnessMap": "粗糙度贴图",
        "material.shininess": "高光强度",
        "material.specular": "高光",
        "material.specularMap": "高光贴图",
        "material.texture.image": "图片",
        "material.texture.offset": "偏移",
        "material.texture.repeat": "重复",
        "material.texture.rotation": "旋转",
        "material.texture.wrapS": "Wrap S",
        "material.texture.wrapT": "Wrap T",
        "mcp.appUrl": "本页面地址（桥接程序只接受来自此地址的页面）",
        "mcp.autoConnect": "打开 Chili3D 时自动连接",
        "mcp.bridgeAt": "桥接程序：{0}",
        "mcp.bridgeCommand": "命令（仅在使用分支或本地构建时修改）",
        "mcp.bridgeHint":
            "桥接程序是本机上由智能体启动的小程序。无需克隆或构建任何代码：选择以下两种方式之一获取。",
        "mcp.claudeCode": "Claude Code：在终端中运行",
        "mcp.connect": "连接",
        "mcp.connectHint":
            "重启智能体（或重新加载其 MCP 服务器），然后点击上方的“连接”。此标签页连接后，智能体即可看到 Chili3D 工具。智能体工作期间请保持此标签页打开。",
        "mcp.copied": "已复制",
        "mcp.copy": "复制",
        "mcp.disconnect": "断开",
        "mcp.download": "下载：",
        "mcp.executableHint":
            "可放在任意位置，例如用户主目录。在 macOS 和 Linux 上，先执行一次 chmod +x <文件> 使其可执行。在 macOS 上，如果因“来自身份不明的开发者”而被阻止，请运行 xattr -d com.apple.quarantine <文件>。",
        "mcp.executableMissing": "请在第 2 步中填写可执行文件的保存位置：下方命令仍包含占位路径。",
        "mcp.executablePath": "保存位置的完整路径",
        "mcp.generate": "生成",
        "mcp.intro":
            "让本机上的 AI 智能体（Claude Code、Claude Desktop、Cursor 等）通过模型上下文协议（MCP）在此标签页中建模。智能体会启动一个小型桥接程序，本页面连接到它并在浏览器内执行工具，文档不会离开浏览器。",
        "mcp.jsonConfig": "Claude Desktop、Cursor 等客户端：添加到 MCP 配置文件",
        "mcp.noTokenWarning":
            "不安全：没有令牌时，本机上的任何程序都能连接桥接端口并控制此标签页（读取、修改、导出模型）。网站仍会被来源检查拦截。仅在可信的计算机上使用。",
        "mcp.nodeHint":
            "需要 Node.js 20 或更高版本（nodejs.org）。智能体首次启动时，npx 会从本站下载桥接程序。",
        "mcp.offlineHint": "无法连接桥接程序。请确认智能体已启动它，且其端口和令牌与下方设置一致。正在重试…",
        "mcp.port": "端口",
        "mcp.remoteHint":
            "本页面来自其他服务器，浏览器可能会询问是否允许访问本地网络上的应用或设备：请选择允许，否则无法连接桥接程序。Chrome、Edge 和 Firefox 支持此功能；Safari 会阻止。",
        "mcp.requireToken": "需要配对令牌（推荐）",
        "mcp.runner.executable": "下载可执行文件（无需安装其他内容）",
        "mcp.runner.node": "使用 Node.js 运行（npx 从本站获取）",
        "mcp.status.connected": "已连接：智能体可以控制此标签页",
        "mcp.status.connecting": "连接中…",
        "mcp.status.idle": "未连接",
        "mcp.status.offline": "等待桥接程序",
        "mcp.step.bridge": "2. 桥接程序",
        "mcp.step.connect": "4. 连接",
        "mcp.step.register": "3. 在智能体中注册桥接程序",
        "mcp.step.token": "1. 配对令牌",
        "mcp.title": "MCP",
        "mcp.tokenHint":
            "只有提供相同令牌时桥接程序才会接受此页面。令牌已自动生成并保存在此浏览器中；若重新生成，请将更新后的配置再次复制到智能体中。",
        "model.visible": "可见",
        "option.command.circularPattern": "弧形阵列",
        "option.command.continuity": "连续性",
        "option.command.deleteObjects": "删除对象",
        "option.command.depth": "深度",
        "option.command.externalRole": "角色",
        "option.command.externalRole.profile": "轮廓",
        "option.command.externalRole.reference": "参考",
        "option.command.insertPoint": "插入点",
        "option.command.intersection": "相交",
        "option.command.isConnected": "相连",
        "option.command.isConvertInstance": "转为实例",
        "option.command.isFace": "面",
        "option.command.isRoundCorner": "圆角",
        "option.command.isRuled": "直纹曲面",
        "option.command.isSolid": "实体",
        "option.command.joinType": "连接类型",
        "option.command.joinType.arc": "圆弧",
        "option.command.joinType.intersection": "相交",
        "option.command.joinType.tangent": "相切",
        "option.command.merge": "合并",
        "option.command.modifyBoundary": "同时修改边界",
        "option.command.normalOffset": "法向间距",
        "option.command.offsetMode": "偏移模式",
        "option.command.offsetMode.pipe": "管道",
        "option.command.offsetMode.rectoVerso": "正反面",
        "option.command.offsetMode.skin": "蒙皮",
        "option.command.operation": "操作",
        "option.command.operation.cut": "移除",
        "option.command.operation.intersect": "相交",
        "option.command.operation.join": "加入",
        "option.command.operation.new": "新建",
        "option.command.patternType": "阵列类型",
        "option.command.patternType.circular": "环形阵列",
        "option.command.patternType.curve": "沿曲线阵列",
        "option.command.patternType.linear": "线形阵列",
        "option.command.patternType.rectangular": "矩形阵列",
        "option.command.pitch": "螺距",
        "option.command.repeat": "重复",
        "option.command.startOffset": "起始偏移",
        "option.command.symmetric": "对称",
        "option.command.thickness": "厚度",
        "option.command.totalLength": "总长",
        "option.rect.centerRect": "中心矩形",
        "point.position": "位置",
        "polygon.points": "点",
        "regularPolygon.sides": "边数",
        "pipe.bendRadius": "弯管半径",
        "pipe.bendType": "弯管类型",
        "prompt.default{0}{1}": "{0} 平移视图，{1} 旋转视图，滚轮缩放视图",
        "prompt.deleteDocument{0}": "是否删除 {0} ？",
        "prompt.pickArcBulge": "请选择凸点，按 ESC 键取消",
        "prompt.pickArcEnd": "请选择终点，按 ESC 键取消",
        "prompt.pickArcHeight": "请选择圆弧高度，按 ESC 键取消",
        "prompt.pickArcMid": "请选择圆弧上的点，按 ESC 键取消",
        "prompt.pickCircleCenter": "请选择圆心  按 ESC 键取消",
        "prompt.pickDimensionPosition": "请选择标注位置，按 ESC 键取消",
        "prompt.pickExternalRef": "请选择外部参考（紫色）以切换其角色，按 ESC 键结束",
        "prompt.pickFistPoint": "请选择第一个点, 按 ESC 键取消",
        "prompt.pickNextPoint": "请选择下一个点， 按 ESC 键取消",
        "prompt.pickPoint": "选择点，ESC键取消",
        "prompt.pickRadius": "请选择半径，按 ESC 键取消",
        "prompt.pickSketchEntity": "请选择草图实体，按 ESC 键取消",
        "prompt.pickSketchPoint": "请选择草图点，按 ESC 键取消",
        "prompt.polygon.close": "闭合",
        "prompt.dragToExtrude": "拖拽箭头设置拉伸距离，单击轮廓面可切换，按 ESC 键取消",
        "prompt.saveDocument{0}": "是否保存对 {0} 的更改？",
        "prompt.select.axis": "请选择轴",
        "prompt.select.boundary": "请选择边界边",
        "prompt.select.curve": "请选择曲线",
        "prompt.select.edges": "请选择边",
        "prompt.select.extendTarget": "请选择要延伸的边",
        "prompt.select.faces": "请选择面",
        "prompt.select.models": "请选择模型",
        "prompt.select.plane": "请选择基准平面或实体平面",
        "prompt.select.noModelSelected": "未选择任何模型",
        "prompt.select.openFaces": "请选择要开口的面",
        "prompt.select.path": "请选择路径",
        "prompt.select.section": "请选择截面",
        "prompt.select.shape": "选择形状",
        "prompt.select.sketch": "选择草图",
        "prompt.select.solids": "请选择实体",
        "prompt.select.vertexes": "请选择点",
        "prompt.select.wires": "请选择线",
        "prompt.selectedCount": "已选择",
        "properties.group.transform": "转换",
        "properties.header": "属性",
        "features.consumeTools": "移除工具",
        "features.header": "特征",
        "features.reselect": "重新选择",
        "features.suppress": "抑制",
        "features.unsuppress": "取消抑制",
        "properties.multivalue": "多个值",
        "rect.dx": "长",
        "rect.dy": "宽",
        "ribbon.group.2d": "2D",
        "ribbon.group.3d": "3D",
        "ribbon.group.act": "场景",
        "ribbon.group.annotation": "注释",
        "ribbon.group.constraint": "约束",
        "ribbon.group.dimension": "标注",
        "ribbon.group.group": "组",
        "ribbon.group.modify": "修改",
        "ribbon.group.other": "其他",
        "ribbon.group.plugin": "插件",
        "ribbon.group.selection": "选择",
        "ribbon.group.tools": "工具",
        "ribbon.tab.edit": "编辑",
        "ribbon.tab.file": "文件",
        "ribbon.tab.plugin": "插件",
        "ribbon.tab.sketch": "草图",
        "sketch.conflicting": "约束冲突",
        "sketch.constraintExists": "约束已存在",
        "sketch.externalRefAssociativeOnly": "外部参考仅支持关联约束",
        "sketch.externalRefsLost{0}": "{0} 个草图外部参考丢失了源，轮廓将使用冻结的几何",
        "sketch.externalRefTypeChanged": "外部参考类型已更改——其相关约束已被移除",
        "sketch.fullyConstrained": "完全约束",
        "sketch.noProjectableEdges": "没有可投影的共面边线",
        "sketch.rollbackFailed": "部分实体无法回滚到草图的时间线位置",
        "sketch.underConstrained": "欠约束",
        "snap.center": "圆心",
        "snap.end": "端点",
        "snap.intersection": "交点",
        "snap.mid": "中点",
        "snap.nearest": "最近点",
        "snap.perpendicular": "垂点",
        "snap.nearCurve": "在边线上",
        "snap.onSurface": "在面上",
        "snap.tangent": "切点",
        "statusBar.snap": "捕捉",
        "statusBar.tracking": "追踪",
        "toast.command.{0}excuting": "{0}命令正在执行",
        "toast.converter.error": "转换错误",
        "toast.converter.invalidColor": "无效的颜色",
        "toast.consumedTool.forbidden":
            "布尔工具已被参数体消耗，无法复制/移动/删除——请通过所属参数体的特征列表进行编辑",
        "toast.delete{0}Objects": "删除了 {0} 个对象",
        "toast.document.noActivated": "未打开任何文档",
        "toast.document.saved": "文档已保存",
        "toast.downloading": "正在下载",
        "toast.excuting{0}": "正在执行{0}",
        "toast.fail": "失败",
        "toast.read.error": "读取错误",
        "toast.select.noSelected": "未选择任何对象",
        "toast.snap.notFoundValidPoint": "未找到有效的点",
        "toast.success": "成功",
        "transform.rotation": "旋转",
        "transform.scale": "缩放",
        "transform.translation": "位移",
        "variable.add": "添加",
        "variable.addDescription": "添加描述",
        "variable.delete": "删除",
        "variable.description": "描述",
        "variable.descriptionPlaceholder": "可选",
        "variable.empty": "还没有参数 —— 添加一个即可按名称驱动尺寸与特征。",
        "variable.expressionPlaceholder": "50 或 width / 2",
        "variable.moveDown": "下移",
        "variable.moveUp": "上移",
        "variable.namePlaceholder": "length",
        "variable.type": "类型",
        "variable.type.angle": "角度",
        "variable.type.length": "长度",
        "variable.type.unitless": "无量纲",
        "variable.value": "值",
        "variables.title": "参数",
        "vertex.point": "点",
        "viewport.fitContent": "适应内容",
        "viewport.orthographic": "正交视图",
        "viewport.perspective": "透视视图",
        "viewport.zoomIn": "放大",
        "viewport.zoomOut": "缩小",
        "viewport.grid": "显示网格",
        "viewport.mode.solid": "实体",
        "viewport.mode.wireframe": "线框",
        "viewport.mode.solidAndWireframe": "实体和线框",
        "warning.script.fromDomain": "是否信任来之以下域名的脚本？",
        "command.edit.commandSearch": "命令搜索",
        "commandSearch.placeholder": "搜索命令…",
        "ribbon.group.assemble": "装配",
        "ribbon.group.construct": "构造",
        "ribbon.group.convert": "转换",
        "ribbon.group.create": "创建",
        "ribbon.group.finish": "完成草图",
        "ribbon.group.insert": "插入",
        "ribbon.group.inspect": "检查",
        "ribbon.tab.solid": "实体",
        "ribbon.tab.surface": "曲面",
        "ribbon.tab.utilities": "实用工具",
    },
} satisfies Locale;
