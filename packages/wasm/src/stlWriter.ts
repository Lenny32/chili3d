...
function writeBinary(meshes: StlMesh[], triangles: number): Uint8Array {
    ...
    writeBinaryHeader(view, triangles);
...
function writeBinaryHeader(view: DataView, triangles: number): void {
    // 80-byte header. Must NOT start with "solid" (that marks an ASCII file).
    const header = "Binary STL generated by Spicy3D";
...
/** Appends one mesh's triangles at `offset`, returning the offset just past them. */
function writeMesh(view: DataView, mesh: StlMesh, offset: number): number {
    ...
    for (let t = 0; t < tris; t++) {
        ...
        offset = writeVec(view, triangleNormal(a, b, c), offset);
        offset = writeVec(view, a, offset);
        offset = writeVec(view, b, offset);
        offset = writeVec(view, c, offset);
...
function writeVec(view: DataView, v: readonly [number, number, number], offset: number): number {
...
export function meshesToStl(meshes: StlMesh[], options?: StlExportOptions): Uint8Array {
    ...
    return writeAscii(meshes, options?.name ?? "spicy3d");
}
